...
public class ModelView implements PropertyContainer, Serializable {
  ...
  private void addVisual(ModelElement e, Visual v) {
    ...
    visualElements.put(e,v);
    modelElements.put(v,e);
    
    int zOrder = nextHighestZOrder();
    zOrderedVisuals.put(zOrder,v);
    visualZOrders.put(v,zOrder);
    
    /* notify all listeners in the UI */
    ui.visualElementAddedToActiveModelView(e, v);
   ...
   * that is selected
   */
  private void drawVisual(Visual v, Rectangle drawingArea, boolean parentInFocus, boolean parentSelected) {
    boolean selected = selectedVisuals.contains(v);
    if(v.getShape().intersects(drawingArea)) {
    ...
    if(v.hasSelectableChildren()) {
      ...
      for(Visual c : v.getSelectableChildren()) {
        drawVisual(c, drawingArea, inFocus, selected);
      }
    }
  }
  
  private Rectangle drawingArea() {
    int w = viewSize.width/2;
    int h = viewSize.height/2;
    return new Rectangle(viewCenter.x - w,viewCenter.y - h, viewSize.width, viewSize.height);
  }
  ...
  private void drawVisuals() {
    for(Visual v : zOrderedVisuals.values()) {
      drawVisual(v, drawingArea(), false, false);
    }
  }

...
public abstract class Visual implements PropertyContainer, AMLWritable {
  ...
  public Visual(Graphics2D superGraphics, Rectangle bounds) {
    this.visualListeners = new HashSet<>();
    this.bounds = bounds;
  ...
  public boolean addVisualListener(VisualListener listener) {
    return visualListeners.add(listener);
  }
  
  public boolean removeVisualListener(VisualListener listener) {
    return visualListeners.remove(listener);
  }
  
  protected void dataHasChanged() {
    for(VisualListener listener : visualListeners) {
      listener.visualDataChanged(this);
    }
  }
  ...
  public AMLNode getAMLNode() {
    ...
    node.putAttribute("boundsX", bounds.x);
    node.putAttribute("boundsZ", bounds.y);
    node.putAttribute("boundsWidth", bounds.width);
    node.putAttribute("boundsHeight", bounds.height);
    return node;
  }

  @Override
  public void readAMLNode(AMLNode node) {
    int x = node.getAttributeInt("boundsX");
    int y = node.getAttributeInt("boundsZ");
    int width = node.getAttributeInt("boundsWidth");
    int height = node.getAttributeInt("boundsHeight");
